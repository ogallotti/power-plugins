...
**Spec:** `docs/team-powers/specs/2026-02-19-visual-brainstorming-refactor-design.md`
...
Search the codebase:
```bash
grep -r "wait-for-feedback" --include="*.js" --include="*.md" --include="*.sh" --include="*.json"
```
...
```bash
grep -r "wait-for-feedback\|sendToClaude\|feedback-footer\|send-to-claude\|TaskOutput.*block.*true" --include="*.js" --include="*.md" --include="*.sh" --include="*.html" | grep -v node_modules | grep -v RELEASE-NOTES | grep -v "\.md:.*spec\|plan"
```
